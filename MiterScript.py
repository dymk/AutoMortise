...
class MiterCommand():
    ...
    def tryPlacingTabProfiles(self, fromFace: adsk.fusion.BRepFace, toFace: adsk.fusion.BRepFace) -> bool:
        ...
        # delete existing sketch (if it exists)
        sketchName = ids.TAB_SKETCH_PREFIX + prettyName
        if component.sketches.itemByName(sketchName):
            component.sketches.itemByName(sketchName).deleteMe()
        ...
        # all generated profiles are extudable
        sketch.isComputeDeferred = False
    ...
    def getCandidateFacePairs(
        ...
        for f1 in body1.faces:
            for f2 in body2.faces:
                coplanar = geom.arePlanesCoplanar(f1, f2)
                if coplanar:
